...
class MyPoolPager(
    ...
    sealed interface LoadResult {
        data class Appended(val pageAt: Int) : LoadResult
        data class EndReached(val pageAt: Int) : LoadResult
        data class NoChange(val pageAt: Int) : LoadResult
    ...
    suspend fun loadUntilAppend(
        ...
        while (!done && hops < PREFETCH_AHEAD_PAGES) {
            ...
            outcome = attempt.fold(
                onSuccess = { result ->
                    val items = result.items // ✅ OrdersPage.items
